...
void solve(int ncase) {
  ...
  if (k <= C) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i >= k - 1) os.insert(tmp);
    }
  } else {
    int last = -1; // last one that not in the number
    typedef ModInt<(int) 1e6> MI6;
    MI6 pw(2);
    pw.pow(k);
    //MI6 tmp = 0;
    for (int i = 0; i < n; i++) {
      ...
      debug(i, (i - last < k), (i < k - 1));
      if (i - last < k || i < k - 1) continue;
      int tmp = 0;
      for (int j = i - C; j <= i; j++) tmp = tmp * 2 + s[j];
      debug(i, tmp);
      os.insert(tmp);
    }
  }
  ...
  int k0 = min(k, C);
  //debug(k0);
  ll limit = 1LL << (min(k, C));
  //debug(limit);
  for (int h = 0; h < limit; h++) {
    if (os.find(h) != os.end()) continue;
    puts("YES");
    for (int i = k - 1; i >= k0; i--) putchar('0');
    for (int i = k0 - 1; i >= 0; i--) putchar('0' + ((h & (1 << i)) ? 1 : 0));
    puts("");
    return;
  }
  puts("NO");
