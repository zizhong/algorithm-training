/*
* An optimzation is to use a vector counting instead of multiset.
* The obersvation is that everytime the returned set occupies a consecutive range of depth.
* Vector counting can store the relative values more effecient.
**/
#include <vector>
#include <cstdio>
#include <cstring>
...
struct Trie {
  ...
  vector<int> dfs(TrieNode *r) {
    if (!r) return {};
    vector<int> ret(1, r->is_end);
    for (int i = 0; i < 26; i++) {
      if (!r->next[i]) continue;
      vector<int> ch = dfs(r->next[i]);
      for (int j = 0; j < ch.size(); j++) {
        if (j + 1 >= ret.size()) ret.push_back(0);
        ret[j + 1] += ch[j];
      }
    }
    if (r != root && !r->is_end) {
